...
const Thankyou = ({ currentUserCartItems, currentUser, clickCheckOut,setItems}) => {
  const [cartData, setCartData] = useState(getData("all_users_cart_data"));
  const newData = cartData.filter((allUsers) => {return allUsers.email !== currentUser.email})
  useEffect(()=>{
      setItems(newData);
  },[])
  const showReceipt = cartData.filter((allUsers) => {return allUsers.email === currentUser.email})
  return (
  <div>
